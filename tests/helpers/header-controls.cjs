...
app.whenReady().then(async()=>{
 ...
 await win.webContents.executeJavaScript('(async()=>{for(const source of ["local","meeting"]){emit({...current,source,phase:"stopped",version:current.version+1});await tick();const done=header.shadowRoot.querySelector(".done-button");check(done,"upstream Done control retained");done.click();await tick();check(commands.at(-1)==="Done","Done uses existing lifecycle action");check(header.shadowRoot.querySelector(".listen-button"),"restart stays available")}})()');
 });
 await run('settings supports click and accessible naming in meeting-only mode',async()=>{
 await win.webContents.executeJavaScript('(async()=>{settingsOpened=0;const button=header.shadowRoot.querySelector(".settings-button");check(button.getAttribute("aria-label")==="Settings","Settings has accessible name");button.click();await tick();check(settingsOpened===1,"click opens normal Settings")})()');
 });
