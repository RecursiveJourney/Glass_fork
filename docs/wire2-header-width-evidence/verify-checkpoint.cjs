...
for(const root of [glass,parent]){
 ...
 const allowed=file=>root===glass
  ? /^(?:src\/window\/(?:windowBounds|windowResize|windowLayoutManager|windowManager|smoothMovementManager)\.js|src\/ui\/listen\/ListenView\.js|src\/ui\/app\/(?:MainHeader|HeaderController|WelcomeHeader|ApiKeyHeader|PermissionHeader)\.js|tests\/(?:window-attachment|listen-resize-ack|native-resize|header-controls)\.test\.js|tests\/helpers\/(?:meeting-fixture\.html|meeting-renderer\.cjs|native-resize\.cjs|header-controls\.cjs)|docs\/(?:meeting-feed-subscriber\.md|wire2-(?:phase4|resize|header|header-width)-checkpoint\.md|wire2-(?:phase4|resize|header|header-width)-evidence\/[^/]+))$/.test(file)
...
console.log('PASS: changes limited to approved header width/restoration/resize code, tests, docs and prior Phase 4 work; listener source/tests, Ask, capture, repositories and provider settings unchanged.');
