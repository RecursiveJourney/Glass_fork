...
for(const root of [glass,parent]){
 ...
 const allowed=file=>root===glass
  ? /^(?:src\/window\/(?:windowBounds|windowResize|windowLayoutManager|windowManager|smoothMovementManager)\.js|src\/ui\/listen\/ListenView\.js|src\/features\/listen\/listenService\.js|src\/ui\/app\/(?:MainHeader|HeaderController|WelcomeHeader|ApiKeyHeader|PermissionHeader)\.js|tests\/(?:window-attachment|listen-resize-ack|native-resize|header-controls|meeting-lifecycle)\.test\.js|tests\/helpers\/(?:meeting-fixture\.html|meeting-renderer\.cjs|native-resize\.cjs|header-controls\.cjs)|docs\/(?:meeting-feed-subscriber\.md|wire2-(?:phase4|resize|header|header-width|live-issues)-checkpoint\.md|wire2-(?:phase4|resize|header|header-width|live-issues)-evidence\/[^/]+))$/.test(file)
 ...
 const text=[git(root,['diff','--no-ext-diff','--no-textconv']).split('\n').filter(l=>l.startsWith('+')&&!l.startsWith('+++')).join('\n'),...fresh.filter(f=>!f.endsWith('.png')).map(f=>fs.readFileSync(path.join(root,f),'utf8'))].join('\n');
...
console.log('PASS: added diff lines and '+count+' untracked files checked (PNG screenshots excluded from text scan); no configured credential or common key-pattern matches.');
console.log('PASS: changes limited to approved Meeting layout/color and earlier header/resize code, tests, docs and prior Phase 4 work; listener source/tests, Ask, capture, repositories and provider settings unchanged.');
